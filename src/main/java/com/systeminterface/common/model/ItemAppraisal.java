 ...
 *
 * <p>TODO: Replace this with a richer item model that considers utility,
 * teleports, progression relevance, economy/value, combat power, skilling use,
 * quest/achievement role, and use case instead of leaning mostly on material
 * tier and GE value. Utility items such as teleport jewellery can be much more
 * useful than their current heuristic rank implies.
 */
public final class ItemAppraisal
{
	...
	public static ItemAppraisal appraise(String name, int value, boolean equipable, int equipSlot, boolean noted)
	{
		...
		if (noted)
		{
			...
			final String baseName = lower.endsWith(" (noted)")
				? name.substring(0, Math.max(0, name.length() - " (noted)".length()))
				: name;
			return new ItemAppraisal(valueRank, ItemClass.NOTED,
				"Noted form of " + baseName + ". Trade or bank to use.", null);
		}
